...
if(isset($_POST['save'])){
    ...
    $_SESSION['date'];
        ...
        // $qurty=mysqli_query($conn,"select * from st_sub  where st_id ='$_SESSION[userId]'");
        // $count = mysqli_num_rows($qurty);
        // if($count == 0){ 
        ...
        //     // $qus=mysqli_query($conn,"SELECT * FROM st_sub where st_id='$_SESSION[userId]'");
        //     // $qus=mysqli_query($conn,"SELECT st_id FROM students where st_id not in(SELECT st_id from st_sub) and st_id='$_SESSION[userId]'");
        //     $qus=mysqli_query($conn,"SELECT sub_id FROM subject");
        //     while ($ros = $qus->fetch_assoc())
        //     {
        ...
        //         $qquery=mysqli_query($conn,"insert into st_sub(st_id,sub_id) values('$_SESSION[userId]','0')")or die("not inserted 1");
        ...
        // if($count == 0){ 
            ...
            $status = "";
                ...
                if(isset($check[$i])) //the checked checkboxes
                {
                    ...
                    //   $qquery=mysqli_query($conn," update st_sub set sub_id=' $admissionNo[$i]' where sub_id ='$c' ")or die("not");
                         $qquery=mysqli_query($conn,"insert into st_sub(st_id,sub_id) values('$_SESSION[userId]','$check[$i]')")or die("not inserted 1");
       
                      if ($qquery==true) {

                          $statusMsg = "<div class='alert alert-success'  style='margin-right:700px;'>Subjects Alotted Successfully!</div>";
                      }
                      else
                      {
                          $statusMsg = "<div class='alert alert-danger' style='margin-right:700px;'>An error Occurred!</div>";
          ...
          <div class="row">
            ...
            <div class="card mb-4">
                <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
                  <h6 class="m-0 font-weight-bold text-primary">Select Subjects</h6>
                ...
                <div class="card-body">
                  ...
                  <div class="col-xl-4">
                    ...
                    <?php
                      ...
                      ?>
                    <button type="submit" name="view" class="btn btn-primary">View Subjects</button>
                  </form>
                </div>
              </div>
                ...
                <?php echo $statusMsg; ?>
                  ...
                  <?php
                    ...
                    if(isset($_POST['view'])){
                        ...
                        ?>
                          <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
                        <h6 class="m-0 font-weight-bold text-primary">All Available Subject </h6>
                        
                        <h6 class="m-0 font-weight-bold text-danger">Note: <i>Click on the checkboxes besides each Subject to Select!</i></h6>
                        </div> 
                <?php
                        ...
                        while ($rows = $rs->fetch_assoc())
                          {
                            ...
                            echo"
                              <tr>
                                ...
                                <td>".$rows['tc_name']." ".$rows['tc_lname']."</td>
                            
                                <td><input name='check[]' type='checkbox' value=".$rows['sub_id']." class='name' ></td>
                              </tr>";
                              echo "<input name='admissionNo[]' value=".$rows['sub_id']." type='hidden' class='form-control form-control-sm'>";
                          }
                      }
                      else
                      {
