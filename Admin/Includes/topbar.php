...
<?php 
  // $query = "SELECT * FROM admininfo WHERE username = {$_SESSION['userId']}";
  // $rs = $conn->query($query);
  // $num = $rs->num_rows;
  // $rows = $rs->fetch_assoc();
  // $fullName = $rows['fname']." ".$rows['lastName'];
  


$query = "SELECT * FROM admininfo WHERE id = ".$_SESSION['userId']."";
$rs = $conn->query($query);
$num = $rs->num_rows;
$rows = $rs->fetch_assoc();
$fullName = $rows['fname'];
          ...
          <ul class="navbar-nav ml-auto">
            ...
            <li class="nav-item dropdown no-arrow">
              <a class="nav-link dropdown-toggle" href="#" id="userDropdown" role="button" data-toggle="dropdown"
                aria-haspopup="true" aria-expanded="false">
                <img class="img-profile rounded-circle" src="img/logOutIcon.png" style="max-width: 60px">
