...
if(isset($_POST['save'])){
    ...
    $N = count($admissionNo);
    $status = "";
    $dateTaken=$_POSt['dateTake'];

//check if the attendance has not been taken i.e if no record has a status of 1
  $qurty=mysqli_query($conn,"select * from attendance  where st_status='1' and stat_date='$dateTaken' ");
  $count = mysqli_num_rows($qurty);
  ...
  if($count > 0){
    ...
    else //update the status to 1 for the checkboxes checked
    {
        ...
        for($i = 0; $i < $N; $i++)
        {
                ...
                {
                      $qquery=mysqli_query($conn," update attendance set st_status='1' where stat_id = '$check[$i]' ")or die("not");
...
<!--                       
                      ...
                      <tr>
                        ...
                        <th><input type="checkbox" class="checkbox" id="checkAll"  /> All Present</th>
                        <!-- <th >Check</th> -->
